...
import { useDeviceSettingViewTransition } from '../09-01-device-setting-view-transition/hooks';
...
export const useRoutingSettingViewTransition = () => {
  ...
  const onRoutingBack = () => {
    ...
    document.documentElement.classList.add('backBtn');
    document
      .startViewTransition(() => {
        router.back();
      })
      .finished.finally(() => {
        document.documentElement.classList.remove('backBtn');
      });
  };
